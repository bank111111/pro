...
const FrequencyInformation = ({
  ...
  const datasetTooltip = (
    <div style={{ padding: '8px', fontSize: '13px', lineHeight: '1.6' }}>
      <div style={{ marginBottom: '8px' }}>Maximum 5 Dataset Triggers can be added</div>
        ...
        <div>
          ...
          <div className="flex gap-4">
            ...
            <div className="relative flex-1">
              <button
                className={`w-full px-6 py-3 text-white font-medium rounded transition-colors ${
                  datasetTriggers.length >= 5
                    ? 'bg-gray-400 cursor-not-allowed'
                    : 'bg-blue-500 hover:bg-blue-600'
                }`}
                onClick={() => datasetTriggers.length < 5 && setActiveModal('dataset')}
                disabled={datasetTriggers.length >= 5}
              >
                Add Dataset Trigger
              </button>
              <div className="absolute -top-2 -right-2">
                ...
                <tbody>
                  ...
                  {schedules.map((schedule, index) => {
                    ...
                    const days = schedule.runDays?.length > 0 
                      ...
                      : '';
                    const holidayNote = schedule.holidayAction === 'do-not-run' 
                      ? '(DA) - Job does not run on the day after holiday'
                      : '';
